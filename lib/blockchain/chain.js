...
Chain.prototype._add = co(function* add(block, flags) {
  ...
  while (block) {
    ...
    // Do we already have this block and all its transctions?
    if (yield this.db.hasEntry(hash)) {
      var newtxs = false;
      for (var i = 0; i < block.txs.length; i++) {
        var havetx = yield this.db.hasTX(block.txs[i].hash('hex'));
        if(!havetx) newtxs = true;
      }
      if(!newtxs) {
...
Chain.prototype._getLocator = co(function* getLocator(start) {
  ...
  while (hash) {
    ...
    if (height === this.network.genesis.height) {
...
Chain.prototype.getState = co(function* getState(prev, deployment) {
  console.log("Chain.getState(prev, "+deployment.bit+")")
  ...
  console.log("getState processing "+compute.length+" entries: ", compute)
  while (compute.length) {
    entry = compute.pop();

    switch (state) {
      case thresholdStates.DEFINED:
        console.log("deployment bit "+bit+" DEFINED")
        time = yield entry.getMedianTime();
      ...
      case thresholdStates.STARTED:
        console.log("deployment bit "+bit+" STARTED")
        time = yield entry.getMedianTime();
        ...
        console.log("counting blocks with deployment bit "+bit+" for "+period+" blocks from block "+block.height)
        for (i = 0; i < period; i++) {
          if (block.hasBit(bit))
            count++;
      ...
      case thresholdStates.LOCKED_IN:
        console.log("deployment bit "+bit+" LOCKED_IN")
